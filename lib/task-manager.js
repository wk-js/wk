...
class TaskManager {
  ...
  prerequisites(prereqs, sequence, invocator) {
    const tasks = this._resolve(prereqs)
    ...
    for (let i = 0, ilen = tasks.length; i < ilen; i++) {
      tasks[i].options.invocator = invocator
    }

    return this[`_${sequence}`]( tasks )
  }
  ...
  _serie(tasks) {
    ...
    return when.reduce(tasks, (r, res) => {
      res.options.preReqValues = results
  ...
  _parallel(tasks) {
    ...
    return when.map(tasks, (res) => {
      return this.run(res.task, res.options, res.invocator)
    })
  }
