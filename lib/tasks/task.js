...
class Task {
   ...
   * @memberof Task
   */
  execute(subtaskOrOptions) {
    let subtask = subtaskOrOptions
    if (!(typeof subtaskOrOptions == 'object' && subtaskOrOptions instanceof SubTask)) {
      subtask = this.createSubtask(subtaskOrOptions)
    }
   ...
   * @memberof Task
   */
  invoke(subtaskOrOptions) {
    let subtask = subtaskOrOptions
    if (!(typeof subtaskOrOptions == 'object' && subtaskOrOptions instanceof SubTask)) {
      subtask = this.createSubtask(subtaskOrOptions)
    }

    subtask.options.prerequisites = true
  ...
  /**
   * Execute subtasks in concurrency
   *
   * @memberof Task
   */
  _next() {
    if (this.options.concurrency > -1 && this._running.length >= this.options.concurrency) return
