...
class SubTask {
  ...
  get fullname() {
    return this.task.name + '_' + this.guid
  }

  get name() {
    return this.task.name + '_' + this.guid.slice(0, 4)
  }
  ...
  _execute() {
    ...
    if (canExecute) {
      Print.debug('Execute ' + Print.magenta(`[${this.task.path}]`))
  ...
  _generatePromise() {
    ...
    this.promise.catch((value) => {
      ...
      Print.error( value instanceof Error ? value.stack : value )
    })
  }
