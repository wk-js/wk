...
class SubTask {
  ...
  constructor(task, options) {
    ...
    this.taskValues = options.taskValues
  ...
  _execute() {
    const canExecute = typeof this.task.options.action === 'function' && SubTask.STATUS.IS_PROCESSING(this.status)

    if (canExecute) {
      wk.Print.debug('Execute ' + wk.Print.magenta(`[${this.task.path}]`))

      if (this.task.options.command) {
        this.argv.config = this.argvCommand.config
        this.argv.validate()
      }

      try {
        this.value = this.task.options.action.call(this, this.argv.params)
      } catch(e) {
        this.fail(e)
      }

      if (!this.task.options.async) this.complete( this.value )
