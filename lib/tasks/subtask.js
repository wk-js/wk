...
class SubTask {
  ...
  constructor(task, options) {
    this.complete = scope(this.complete, this)
    this.fail     = scope(this.fail, this)
    this.execute  = scope(this.execute, this)
  ...
  _done() {
    const time = wk.Print.timeEnd(this.guid)
    if (typeof time == 'undefined') return

    wk.Print.debug(wk.Print.magenta(`[${this.path}]`) + ` executed in ${time}`)
  }
  ...
  _prepare() {
    if (this.options.command) {
      ...
      if (this.argv.params.help && this.command.config.help) {
        console.log(this.name, this.command.config.help.description)
        this.fail('Help')
        return false
      }

      if (res.errors) {
        this.fail( res.errors.map(function(error) {
