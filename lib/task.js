...
const { merge, clone, expose } = require('lol/utils/object')
...
class Task {
  ...
  constructor(name, options) {
    ...
    this.command = new ArgvCommand(this.name)
  ...
  invoke(options) {
    ...
    if (Array.isArray(this.prerequisites) && this.prerequisites.length > 0) {
      ...
      const p = this.invokePrerequisites(invocator)
      p.catch(this.fail)
      return p.then((preReqValues) => {
