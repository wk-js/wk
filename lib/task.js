...
const { expose, merge, clone, immutable }
...
class Task {
  ...
  get context() {
    return Context.get(this._contextName)
  }
  ...
  executeTask(params) {
    ...
    return this.context.wkno.task.apply(null, presenter)
  }
  ...
  invokeTask(params) {
    if (this.prerequisites.length > 0) {
      return this.context.wkno.task((resolve, reject) => {
        const p = wk[this.options.preReqSequence](this.prerequisites)

        p.then((values) => {
          const presenter = this.presenter(params)
          return this.context.wkno.task.apply(null, presenter).call(null, values)
        })
  ...
  presenter(params) {
    ...
    const options = expose(clone(this.options), [
      'description', 'visible', 'async', 'concurrency'
    ])
    options.taskName = context.path
