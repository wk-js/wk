...
class Importer {
  ...
  load(p, createNamespace) {
    ...
    for (var i = 0, ilen = filePaths.length; i < ilen; i++) {
      const ext = path.extname(filePaths[i])

      if (ext !== '.js')   filePaths.push(filePaths[i]+'.js')
      if (ext !== '.wk')   filePaths.push(filePaths[i]+'.wk')
    }
  ...
  task( name ) {
    const p = path.join(process.env.HOME, '.wk', 'tasks', name)
    this.load(p)
  }
