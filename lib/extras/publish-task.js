...
class PublishTask extends ExtraTask {
  ...
  _configure() {
     ...
     */
    taskProcess('stage_clean', 'git status --porcelain --untracked-files=no', function(ps, err, stdout) {
     ...
     */
    taskProcess('version_released', 'git ls-remote', function(ps, err, stdout) {
      const version = getCurrentVersion()

      if (stdout.match(new RegExp(`\srefs\/tags\/v${version}(\s|$)`))) {
