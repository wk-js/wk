...
const Print     = new (require('wk-print').Print)
...
class PublishTask extends ExtraTask {
  ...
  configure() {
    ...
    taskProcess('stage_clean', 'git status --porcelain --untracked-files=no', function(result) {
      if (result.stdout.length !== 0) return this.fail(`Stage is not clean`)
      this.complete()
    ...
    task('next_version', { visible: false }, function() {
      const ARGV = this.invocator.argv
      ...
      this.invocator.argv.nextVersion = nextVersion
    })
    ...
    task('confirm_version', { async: true, visible: false }, function() {
      const nextVersion = this.invocator.argv.nextVersion
    ...
    task('commit', { async: true, visible: false }, function() {
      ...
      wk.exec(cmds).catch(this.fail).then(() => {
        Print.log(`Version ${nextVersion} bumped !`)
        this.complete()
      })
    })
     ...
     */
    taskProcess('version_released', 'git ls-remote', function(result) {
